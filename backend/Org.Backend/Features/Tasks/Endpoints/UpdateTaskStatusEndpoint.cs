...
public class UpdateTaskStatusEndpoint : Endpoint<UpdateTaskStatusRequest, ApiResponse<TaskDto>>
{
    private readonly ITaskService _taskService;

    public UpdateTaskStatusEndpoint(ITaskService taskService)
    {
    ...
    public override async Task HandleAsync(UpdateTaskStatusRequest req, CancellationToken ct)
    {
        try
        {
            ...
            var result = await _taskService.UpdateTaskStatusAsync(taskId, req, userId, ct);
        ...
        {
            HttpContext.Response.StatusCode = 400;
            Response = ApiResponse<TaskDto>.ErrorResponse("Failed to update task status", new List<string> { ex.Message });
        }
    }
}
