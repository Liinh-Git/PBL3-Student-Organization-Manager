...
public class UpdateTaskEndpoint : Endpoint<UpdateTaskRequest, ApiResponse<TaskDto>>
{
    ...
    public override async Task HandleAsync(UpdateTaskRequest req, CancellationToken ct)
    {
        try
        {
            ...
            var result = await _taskService.UpdateTaskAsync(taskId, req, userId, ct);
        ...
        {
            HttpContext.Response.StatusCode = 400;
            Response = ApiResponse<TaskDto>.ErrorResponse("Failed to update task", new List<string> { ex.Message });
        }
    }
}
