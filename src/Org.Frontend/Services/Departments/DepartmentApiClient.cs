...
public sealed class DepartmentApiClient(HttpClient httpClient) : IDepartmentService
{
    ...
    {
        var payload = await _httpClient.GetFromJsonAsync<FeatureGetDepartmentsResponse>($"api/organizations/{orgId}/departments")
    ...
    public async Task<DepartmentDto> CreateDepartment(CreateDepartmentRequest req)
    {
        ...
        using var response = await _httpClient.PostAsJsonAsync("api/departments", payload);
    ...
    public async Task<DepartmentDto> UpdateDepartment(Guid id, UpdateDepartmentRequest req)
    {
        ...
        using var response = await _httpClient.PutAsJsonAsync($"api/departments/{id}", payload);
        response.EnsureSuccessStatusCode();
